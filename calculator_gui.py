...
f1 = Frame(root) #containing ( and ! button
f2 = Frame(root) #containing ) and % button

## global variable to restrict putting . (dot) multiple times to reduce chance of syntax error 
global is_dot_there
is_dot_there = False
...
def infx_to_pofx(s):
    ...
    for i in s:
        if i == '(':
            ...
            temp += i   # keep concatinate continuous digit untill operator appears
        elif i == ')':
            if temp != "":
                num_list.append(float(temp))
                temp=""
            ...
            popped = stack.pop() # to remove the left parenthesis 
        else:
            if temp != "":
                num_list.append(float(temp))
                temp=""
...
def pofx_evaluate(s):
    ...
    try:
        for i in s:     
            if i in ['+', '-', '*', '/', '^', '%', '!']:
                operator = i
                # different evaluation process for each different operators
                op2 = float(stack.pop())
                if i == '+':
                    op1 = float(stack.pop())
                    result = op1 + op2
...
def function_backspace():
    ...
    t.delete(1.0, END)
    t.insert(END , val)
    #updating the is_dot_there variable after back-spacing
    for i in val:
        if i == '.':
            is_dot_there = True
        elif i in ['+', '-', '*', '/', '^', '%', '!']:
...
def button_equal():
    ...
    if pressed_equal == False:
        ...
        if res == "Error":
            ...
            t.insert(END , int(res) if (math.floor(res) == math.ceil(res)) else float(f"{res:.10f}"))                             
    pressed_equal = True
...
Button_left_parenthesis = Button(f1, text = "(", padx=48, pady=2, bg='#b3cccc',font = myFont, command=lambda: on_Click('('))
Button_factorial = Button(f1, text = "n!", padx=43, pady=2, bg='#b3cccc',font = myFont, command=lambda: on_Click('!'))

Button_right_parenthesis= Button(f2, text = ")", padx=47, pady=2, bg='#b3cccc',font = myFont, command=lambda: on_Click(')'))
Button_percentage = Button(f2, text = "%", padx=41, pady=2, bg='#b3cccc',font = myFont, command=lambda: on_Click('%'))
...
Button_percentage.pack(side="top")
Button_factorial.pack(side="top")
